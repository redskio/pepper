...
## 레퍼런스 기반 디자인 원칙 (MANDATORY)

Pepper는 결과물을 제작할 때 반드시 해당 유형의 레퍼런스 폴더를 먼저 확인하고, 레퍼런스 스타일에 맞춰 제작한다.
...
C:\Agent\pepper\references\
  ...
  ui_mockup\      ← UI 목업 레퍼런스
```

### 레퍼런스 활용 규칙
1. **작업 시작 전**: 해당 결과물 유형의 레퍼런스 폴더를 확인한다
2. **레퍼런스 있음**: 레퍼런스 파일의 레이아웃, 색상 팔레트, 폰트 스타일, 구성 방식을 최대한 반영하여 제작한다
3. **레퍼런스 없음**: 기본 디자인 원칙으로 제작하고, Boss에게 레퍼런스 추가를 제안한다
4. **새 레퍼런스 추가 시**: Boss가 레퍼런스 파일을 제공하면 해당 유형 폴더에 저장하고 CLAUDE.md에 등록한다
...
## 데이터 사용 원칙 (MANDATORY — 할루시네이션 방지)
제안서, 보고서, 강의자료 등 외부 제출용 결과물 작성 시 아래 규칙을 반드시 준수한다:

1. **수치는 출처 명시 필수**: 모든 통계/지표는 출처 주석을 슬라이드 하단에 표기한다.
   예: "출처: 금융감독원 2023 금융서비스 이용 현황"
2. **미확인 수치 처리**: 검색으로 확인되지 않은 수치는 절대 확정 표현으로 서술하지 않는다.
   허용: "업계 추정치", "약 ~", "*내부자료 필요"
   금지: 출처 없이 정확한 수치를 사실처럼 서술하는 것
3. **추정치 마킹**: 정확한 출처를 찾지 못한 경우 해당 수치 옆에 *(추정)* 또는 *(출처 필요)* 표시를 남긴다.
4. **Strange/Hawkeye 검증 우선**: 제안서에 시장 규모, KPI, ROI 수치가 필요하면 Strange 또는 Hawkeye에게 먼저 데이터를 요청하고, 검증된 수치만 사용한다. 데이터 없이 임의로 채우지 않는다.
