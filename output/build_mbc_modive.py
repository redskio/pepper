...
C_DEEP_PURPLE= RGBColor(0x6B, 0x21, 0xA8)
C_BLUE       = RGBColor(0x1D, 0x4E, 0xD8)
C_DARK       = RGBColor(0x0F, 0x17, 0x2A)
C_LIGHT_PURP = RGBColor(0xF3, 0xE8, 0xFF)
...
C_PURPLE_LIGHT2 = RGBColor(0xD8, 0xB4, 0xFE)
C_MED_PURPLE = RGBColor(0x7C, 0x3A, 0xED)
C_BORDER     = RGBColor(0xE2, 0xE8, 0xF0)
C_DARK_GREY  = RGBColor(0x1E, 0x29, 0x3B)
C_GRAD_MID   = RGBColor(0x44, 0x19, 0x7B)  # gradient midpoint purple
...
OUT_PATH = os.path.join(os.path.dirname(__file__), "mbc_modive_proposal.pptx")
...
def add_header_bar(slide, title_text):
    ...
    add_text_in_shape(bar, title_text, font_size=18, bold=True,
...
def build_slide1(slide):
    ...
    if img_added:
        ...
        overlay = add_rect(slide, Cm(19), 0, Cm(14.87), SLIDE_H,
                           fill_color=RGBColor(0x6B, 0x21, 0xA8))
        sp = overlay.element
        spPr = sp.find(qn('p:spPr'))
        # Set transparency
...
def build_slide4(slide):
    ...
    for i, (num, title, desc) in enumerate(competencies):
        ...
        add_textbox(slide, Cm(16.1), card_y + Cm(0.3), Cm(16), Cm(0.8),
                    title, font_size=15, bold=True, color=C_DEEP_PURPLE)
...
def build_slide7(slide):
    ...
    add_textbox(slide, Cm(1.2), Cm(6.2), Cm(20), Cm(0.7),
                "초기 4개월 우선순위 로드맵", font_size=14, bold=True, color=C_DARK)
...
def build_slide8(slide):
    ...
    add_textbox(slide, Cm(1.2), roi_y, Cm(20), Cm(0.7),
                "ROI 계산", font_size=14, bold=True, color=C_DARK)
...
def build_slide10(slide):
    ...
    add_textbox(slide, Cm(1.2), Cm(1.9), Cm(15), Cm(0.7),
                "패키지 구성", font_size=14, bold=True, color=C_DARK)
    ...
    add_textbox(slide, Cm(17.5), Cm(1.9), Cm(15), Cm(0.7),
                "계약 프로세스", font_size=14, bold=True, color=C_DARK)
...
def build_slide11(slide):
    ...
    quote_box = add_rect(slide, Cm(3), Cm(4), Cm(27.8), Cm(5.5),
                          fill_color=RGBColor(0x3B, 0x0E, 0x6E))
